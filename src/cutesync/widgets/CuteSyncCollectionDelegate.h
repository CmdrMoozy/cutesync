 ...
 *
 * We allow items in the list to be "disabled," so they are greyed out and cannot be clicked,
 * but are still visible in the list. This is used when some action is being performed on a
 * collection that makes it unavailable for more actions to be queued up.
 */
class CuteSyncCollectionDelegate : public QStyledItemDelegate
{
	public:
		...
		const CuteSyncCollectionModel *getModel() const;
		void setModel(const CuteSyncCollectionModel *m);
	
		virtual void paint(QPainter *p, const QStyleOptionViewItem &o, const QModelIndex &i) const;
		
	private:
		const CuteSyncCollectionModel *model;
};
